...
def create_pattern8(pattern8, scene_handler, rubik_camera):
    def print_pattern():
        # on affiche les lettres du motif
        sol_text.text = "U D' R L' F B' U D'"
    ...
    sol_text = Text(origin=(0, 7), scale=2, parent=pattern8)
    # bouton qui permet de retourner au menu des motifs
    Button(parent=pattern8, text='Retour Motif', scale=(0.2, 0.1), position=(0.7, 0.4),
           on_click=Func(setattr, scene_handler, 'state', 'pattern_menu'))
    # bouton qui permet d'afficher la solution du motif
    Button(parent=pattern8, text='Afficher la solution', scale=(0.3, 0.1), position=(0, 0.39), on_click=print_pattern)

    pattern8.on_enable = open_pattern8
    pattern8.on_disable = close_pattern8
